...
Final_table=scores[which(!is.na(scores$tcga_code)),]
Final_table$Cohort=as.character(Final_table$tcga_code)
Final_table=Final_table[which(complete.cases(Final_table$Genome.doublings)),]
...
Results <- data.frame()
for (i in 1:length(colnames(rna_table))) {
...
write.table(Results, "table/wgii_allGeneExp_linear_model_results.txt", quote=F, sep="\t", row.names = F)