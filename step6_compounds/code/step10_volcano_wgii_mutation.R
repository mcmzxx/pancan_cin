...
prow=plot_grid(plot1,plot2, align = "h", axis = "b",ncol=2,rel_widths = c(1,1), labels = c("WGII score vs mutations","SCIN score vs mutations"),label_fontface = 'plain')
...
for (i in 1:length(unique(Final_table$Cohort))) {
  ...
  if (length(which(Final_table$TP53 %in% "mut" &Final_table$Cohort %in% cohort)) >= 20 &
...
plot1=ggplot(Results_TP53, aes(Cohort, Coef,fill=Significant)) +
  ...
  theme_cowplot(12)+ theme(axis.text.x = element_text(size =10, angle = -270, vjust=0.5),
                           axis.text.y=element_text(size=10, colour="black"), legend.position = 'bottom',
                           axis.title=element_text(size=10, colour="black"), 
                           legend.key = element_rect(fill="White"), 
                           legend.text = element_text(size=10), 
                           legend.title = element_text(size=10))
plot1=plot1+scale_fill_manual(values = c("Neg"=tropiccolor[1], "Not Sig"=tropiccolor[2], "Pos"=tropiccolor[3])) 
...
plot2=ggplot(Results_TP53, aes(Cohort, Coef,fill=Significant)) +
  ...
  theme_cowplot(12)+ theme(axis.text.x = element_text(size =10, angle = -270, vjust=0.5),
                           ...
                           legend.text = element_text(size=10), 
                           legend.title = element_text(size=10))
plot2=plot2+scale_fill_manual(values = c("Neg"=tropiccolor[1], "Not Sig"=tropiccolor[2], "Pos"=tropiccolor[3])) 
legend <- get_legend(
  plot2 + theme(legend.box.margin = margin(-1, -1, -1, 0))
)
...
plot1=plot1+theme(legend.position = "none",plot.margin=margin(r=-0.1,t=0.1,l=0.1,unit="cm"))
plot2=plot2+theme(legend.position = "none",plot.margin=margin(t=0.1,unit="cm"))
prow=plot_grid(plot1,plot2, align = "h", axis = "b",ncol=2,rel_widths = c(1,1),labels = c("WGII score vs TP53 Mutation","SCIN score vs TP53 Mutation"),label_fontface = 'plain',label_x = -0.2)
pdf('plot/cin_tp53_mut.pdf.pdf',width =7,height = 6)
plot_grid(prow,legend, align = "v",ncol=1,rel_heights=c(1,0.1))
dev.off()
...
Results = Results[!Results$Gene %in% ".",]
Results$text = "no"
Results$Significant = ifelse(Results$Pvalue < 0.05 & Results$MUTminusWT_diff > 0, "Pos", "Not Sig")
Results$Significant[Results$Pvalue < 0.05 & Results$MUTminusWT_diff < 0] = "Neg"
Results$Significant2 = ifelse(Results$Pvalue < 0.05 & Results$Coef > 0, "Pos", "Not Sig")
Results$Significant2[Results$Pvalue < 0.05 & Results$Coef < 0] = "Neg"
Results$text[which(Results$Significant2!='Not Sig')] = "yes"
Results$text = as.factor(Results$text)
plot1 = ggplot(Results, aes(x = Coef, y = -log10(Pvalue))) +
  geom_point(aes(color = Significant2),size=0.2) +
  xlab("Coefficient (linear model)") +
  ylab("-log10(p-value)") +
  scale_color_manual(values = c("Neg"=tropiccolor[1], "Not Sig"=tropiccolor[2], "Pos"=tropiccolor[3])) +
  theme_cowplot(12)+ theme(axis.text.x=element_text(size=10, colour="black"),  legend.position = 'bottom',
...
plot1=plot1+theme(legend.position = "none",plot.margin=margin(r=-0.1,t=0.1,l=0.1,unit="cm"))
plot2=plot2+theme(legend.position = "none",plot.margin=margin(t=0.1,unit="cm"))
prow=plot_grid(plot1,plot2, align = "h", axis = "b",ncol=2,rel_widths = c(1,1),labels = c("WGII score vs Driver Mutations","SCIN score vs Driver Mutations"),label_fontface = 'plain',label_x = -0.2)
