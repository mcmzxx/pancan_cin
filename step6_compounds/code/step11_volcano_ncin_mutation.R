...
Results <- read.delim("table/ncin_allMutations_linear_model_results.txt")
Results <- Results[order(Results$Pvalue),]
Results <- Results[!Results$Gene %in% ".",]
Results$text <- "no"
Results$text[c(1:10,29)] <- "yes"
...
for(i in 1:length(unique(Final_expression_table_tum$Cohort))){
  ...
  if(length(which(Final_expression_table_tum$TP53 %in% "mut" & Final_expression_table_tum$Cohort %in% cohort)) >=20 & length(which(Final_expression_table_tum$TP53 %in% "mut" & Final_expression_table_tum$Cohort %in% cohort)) >=20){
    ...
    fit <- lm(ncin ~ TP53, Final_expression_table_tum_cohort)
...
name= paste("plot/ncin_tp53_mut.pdf")
...
for(gene in as.character(unique(Mut_table$Hugo_Symbol[Mut_table$driver %in% "TRUE"]))){
  ...
  if(length(which(Final_expression_table_tum$Sample %in% Mut_table$Sample[Mut_table$Hugo_Symbol %in% gene & Mut_table$driver %in% "TRUE"])) >=10 & length(which(!Final_expression_table_tum$Sample %in% Mut_table$Sample[Mut_table$Hugo_Symbol %in% gene & Mut_table$driver %in% "TRUE"])) >=10){
    ...
    fit <- lm(ncin ~ Mutation_selected + Cohort, Final_expression_table_tum)
    
    pvalue = summary(fit)$coefficients[2,4]  
    coef = summary(fit)$coefficients[2,1]
    
    wt <- mean(Final_expression_table_tum$ncin[Final_expression_table_tum$Mutation_selected %in% "WT"])
    mut <- mean(Final_expression_table_tum$ncin[Final_expression_table_tum$Mutation_selected %in% "driver"])
...
write.table(Results, "table/ncin_DriverMutations_linear_model_results.txt", quote=F, sep="\t", row.names = F)


## plot

Results <- read.delim("table/ncin_DriverMutations_linear_model_results.txt")
Results <- Results[order(Results$Pvalue),]
Results <- Results[!Results$Gene %in% ".",]
Results$text <- "no"
...
pdf("plot/ncin_driver_mut.pdf", width = 4.5, height = 5)
