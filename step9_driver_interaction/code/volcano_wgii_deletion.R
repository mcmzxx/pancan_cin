...
Results <- read.delim("table/wgii_allDeletions_linear_model_results.txt")
...
Results <- read.delim("table/scin_allDeletions_linear_model_results.txt")
Results <- Results[order(Results$Pvalue),]
Results <- Results[!Results$Gene %in% ".",]
Results$Significant = ifelse(Results$FDR < 0.05 & Results$MUTminusWT_diff > 0, "positive", "insignificant")
Results$Significant[Results$FDR < 0.05 & Results$MUTminusWT_diff < 0] = "negative"
...
Results$Significant2[Results$FDR < 0.05 & Results$Coef < 0] = "negative"
Results$text = "no"
selt=unique(c(1:5,which(Results$Gene %in% c('PIK3CA','KRAS','BRAF','NRAS', 'HRAS', "AURKB","BUB1",'MYC','RB1','BUB3','AURKA','PTEN')),which(Results$Significant2 %in% c("negative"))[1:5]))
selt=intersect(selt,which(Results$Significant2!="insignificant"))
Results$text[selt] = "yes"
Results$text = as.factor(Results$text)
...
prow=plot_grid(plot1,plot2, align = "h", axis = "b",ncol=2,rel_widths = c(1,1), labels = c("WGII score vs deletions","SCIN score vs deletions"),label_fontface = 'plain',label_x = -0.1)
pdf('plot/lm_cin_deletion.pdf',width =7,height = 6)
