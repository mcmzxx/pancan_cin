...
for(c in 1:length(TCGA_Cohorts)) {
  ...
  if(!all(is.na(Final_table_merged_tumor$OS.time))){
    ...
    high_median <- as.numeric(length(which(complete.cases(Final_table_merged_tumor$OS.time) & complete.cases(Final_table_merged_tumor$OS) & Final_table_merged_tumor$wgii_group %in% "high")))
    low_median <- as.numeric(length(which(complete.cases(Final_table_merged_tumor$OS.time) & complete.cases(Final_table_merged_tumor$OS) & Final_table_merged_tumor$wgii_group %in% "low")))
...
write.table(out, "table/TCGA_OS_analyses_wgii_results.txt", sep="\t", row.names=F, quote=F)
print('wcin')
...
for(c in 1:length(TCGA_Cohorts)) {
  ...
  if(!all(is.na(Final_table_merged_tumor$OS.time))){
    ...
    high_median <- as.numeric(length(which(complete.cases(Final_table_merged_tumor$OS.time) & complete.cases(Final_table_merged_tumor$OS) & Final_table_merged_tumor$scin_group %in% "high")))
    low_median <- as.numeric(length(which(complete.cases(Final_table_merged_tumor$OS.time) & complete.cases(Final_table_merged_tumor$OS) & Final_table_merged_tumor$scin_group %in% "low")))
    surv_summ1 <- summary(km_cluster,times=c(1))
    surv_summ5 <- summary(km_cluster,times=c(1,5))
    lensurv=length(surv_summ5$surv)
    if(lensurv==4 & (min(surv_summ5$n.risk)>2)){
...
pdf('plot/cin_os_sig.pdf',height=9.7,width=6.5)
plot_grid(gtwgii,gtscin,ncol=1,labels=c('A','B'),rel_heights = c(2.1,3))
dev.off()
