...
for(c in 1:length(TCGA_Cohorts)) {
  ...
  if(!all(is.na(Final_table_merged_tumor$PFI.time))){
    Final_table_merged_tumor$PFI.time <- Final_table_merged_tumor$PFI.time/365
    ...
    Final_table_merged_tumor$SurvObj <- with(Final_table_merged_tumor, Surv(Final_table_merged_tumor$PFI.time  , Final_table_merged_tumor$PFI))
    ...
    high_median <- as.numeric(length(which(complete.cases(Final_table_merged_tumor$PFI.time) & complete.cases(Final_table_merged_tumor$PFI) & Final_table_merged_tumor$wgii_group %in% "high")))
    low_median <- as.numeric(length(which(complete.cases(Final_table_merged_tumor$PFI.time) & complete.cases(Final_table_merged_tumor$PFI) & Final_table_merged_tumor$wgii_group %in% "low")))
    ...
    Table_survival_results_Cohort <- data.frame(Cohort=Cohort_TCGA,
                                                sample_number= nrow(Final_table_merged_tumor[complete.cases(Final_table_merged_tumor$PFI.time) & complete.cases(Final_table_merged_tumor$PFI),]),
...
write.table(out, "table/TCGA_PFS_analyses_wgii_results.txt", sep="\t", row.names=F, quote=F)
print('wcin')
...
Final_table_merged_tumor$SurvObj=with(
  ...
  Surv(
    Final_table_merged_tumor$PFI.time,
    Final_table_merged_tumor$PFI
  )
)
...
pl=ggsurvplot_facet(fit=km_cluster,data=Final_table_merged_tumor,facet.by="Cohort",ncol=4,
                    xlab="years",ylab='progression-free survival',size=0.7,
                    censor.size=2.5,
                    pval=TRUE,
                    pval.coord=c(0,0.03),
                    xlim=c(0,10),
...
for(c in 1:length(TCGA_Cohorts)) {
  ...
  if(!all(is.na(Final_table_merged_tumor$PFI.time))){
    ...
    high_median <- as.numeric(length(which(complete.cases(Final_table_merged_tumor$PFI.time) & complete.cases(Final_table_merged_tumor$PFI) & Final_table_merged_tumor$scin_group %in% "high")))
    low_median <- as.numeric(length(which(complete.cases(Final_table_merged_tumor$PFI.time) & complete.cases(Final_table_merged_tumor$PFI) & Final_table_merged_tumor$scin_group %in% "low")))
    surv_summ1 <- summary(km_cluster,times=c(1))
    surv_summ5 <- summary(km_cluster,times=c(1,5))
    lensurv=length(surv_summ5$surv)
    if(lensurv==4 & (min(surv_summ5$n.risk)>2)){
...
Table_survival_results$FDR <- p.adjust(Table_survival_results$pvalue, method="fdr")
out <- Table_survival_results[order(Table_survival_results$pvalue),]
write.table(out, "table/TCGA_PFS_analyses_scin_results.txt", sep="\t", row.names=F, quote=F)
print('scin')
out$Cohort=as.character(out$Cohort)
outres=out[,c("Cohort", "sample_number", "pvalue", "low_surv5", "high_surv5","low_surv5_n", "high_surv5_n")]
outres$Cohort=unlist(lapply(1:length(out$Cohort),function(x){ifelse(out$surv_end[x]==1,paste0(out$Cohort[x],'$^*$'),out$Cohort[x])}))
...
pdf('plot/cin_pfs_sig.pdf',height=8,width=6.5)
plot_grid(gtwgii,gtscin,ncol=1,labels=c('A','B'),rel_heights=c(1,1))
dev.off()
