...
for(c in 1:length(TCGA_cohorts)) {
  ...
  if(!all(is.na(Final_table_merged_tumor$PFI.time))){
    ...
    higher_median <- as.numeric(length(which(complete.cases(Final_table_merged_tumor$PFI.time) & complete.cases(Final_table_merged_tumor$PFI) & Final_table_merged_tumor$scin_group %in% "Higher")))
    lower_median <- as.numeric(length(which(complete.cases(Final_table_merged_tumor$PFI.time) & complete.cases(Final_table_merged_tumor$PFI) & Final_table_merged_tumor$scin_group %in% "Lower")))
    ...
    plot <- ggsurvplot(km_cluster, legend = "none", xlab="Years",ylab='Progression Free Survival',font.main = 18,size=0.2,
...
pr1=plot_grid(UCEC$plot,ACC$plot,KIRP$plot,THCA$plot,ncol = 4)
pr2=plot_grid(PRAD$plot,LGG$plot,CHOL$plot,THYM$plot,ncol = 4)
#pr3=plot_grid(CHOL$plot,KIRC$plot,CESC$plot,ESCA$plot,ncol=4)
#plot_grid(LGG$plot,UCEC$plot,LIHC$plot,BRCA$plot,OV$plot,SARC$plot,TGCT$plot,HNSC$plot,ncol = 4)
pdf('plot/scin_pfs_sig.pdf',height=6,width=10)
