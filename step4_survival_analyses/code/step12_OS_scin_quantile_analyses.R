...
pdf("plot/OS_scin_quantile_plots.pdf", width = 8, height = 8)
for(c in 1:length(TCGA_cohorts)) {
  ...
  if(!all(is.na(Final_table_merged_tumor$DSS.time))){
    ...
    Final_table_merged_tumor$scin_group=cut(Final_table_merged_tumor$scin, quantile(Final_table_merged_tumor$scin+runif(length(Final_table_merged_tumor$scin), -0.01,0.01), seq(0,1,l=5), na.rm=TRUE), right=F,labels=c("0_25%","25_50%","50_75%","75_100%"))
    ...
    f=survfit(msurv~scin_group,data= Final_table_merged_tumor);
    km=coxph(msurv~scin_group,data= Final_table_merged_tumor);
    ...
    sp=ggsurvplot(f, pval = TRUE, conf.int = F,
                  risk.table = TRUE, risk.table.y.text.col = TRUE,xlim=c(0,5),palette ="Set1",break.time.by=1,xlab="Time (years)")+ggtitle(cohort_TCGA)
    print(sp)
    
    
    group_n <- table(Final_table_merged_tumor$scin_group)
...
write.table(out, "table/TCGA_OS_analyses_scin_quantile_results.txt", sep="\t", row.names = F, quote=F)
