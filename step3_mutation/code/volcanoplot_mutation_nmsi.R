...
Results = read.delim("table/wgii_allMutations_linear_model_results_nmsi.txt")
...
Results = read.delim("table/scin_allMutations_linear_model_results_nmsi.txt")
Results = Results[order(Results$Pvalue),]
Results = Results[!Results$Gene %in% ".",]
...
prow=plot_grid(plot1,plot2, align = "h", axis = "b",ncol=2,rel_widths = c(1,1), labels = c("WGII score vs mutations","SCIN score vs mutations"),label_fontface = 'plain',label_x = -0.07)
plot_grid(prow,legend, align = "v",ncol=1,rel_heights=c(1,0.1))
pdf('plot/lm_cin_mutation_nmsi.pdf',width =7,height = 6)
plot_grid(prow,legend, align = "v",ncol=1,rel_heights=c(1,0.1))
dev.off()
...
Final_table=Final_table[which(!(Final_table$Patient %in% msitable$TCGA_Barcode)),]
...
plot1=plot1+theme(legend.position = "none",plot.margin=margin(r=-0.1,t=0.1,l=0.1,unit="cm"))
plot2=plot2+theme(legend.position = "none",plot.margin=margin(t=0.1,unit="cm"))
prow=plot_grid(plot1,plot2, align = "h", axis = "b",ncol=2,rel_widths = c(1,1),labels = c("WGII score vs TP53 mutation","SCIN score vs TP53 mutation"),label_fontface = 'plain',label_x = -0.18)
pdf('plot/cin_tp53_mut_nmsi.pdf',width =7,height = 6)
plot_grid(prow,legend, align = "v",ncol=1,rel_heights=c(1,0.1))
dev.off()
...
Results1=Results
write.table(Results, "table/wgii_DriverMutations_linear_model_results_nmsi.txt", quote=F, sep="\t", row.names = F)
Results=Results2
Results$FDR = p.adjust(Results$pvalue, method = "fdr")
Results = Results[order(Results$pvalue),c(1:3,9,4:8)]
names(Results) = c("Gene", "Coef", "Pvalue", "FDR", "WTmean", "MUT_driver_mean", "MUTminusWT_diff", "WT_samples", "MUT_driver_samples")
Results2=Results
write.table(Results, "table/scin_DriverMutations_linear_model_results_nmsi.txt", quote=F, sep="\t", row.names = F)
Results = Results1
Results = Results[order(Results$Pvalue),]
Results = Results[!Results$Gene %in% ".",]
Results$text = "no"
...
pdf('plot/cin_driver_mut_nmsi.pdf',width =7,height = 6)
plot_grid(prow,legend, align = "v",ncol=1,rel_heights=c(1,0.1))
dev.off()
