...
Results = read.delim("table/wgii_allCnv_metattest_results.txt")
...
highresults=subset(Results, Gene %in% c("GINS1"))
...
plot1=plot1+guides(color=guide_legend(title="significant (FDR < 0.05)",nrow=2,byrow=T))
prow=plot_grid(plot1, align = "h", axis = "b",ncol=1,rel_widths = c(1,1), labels = c("WGII score vs CNVs"),label_fontface = 'plain')
print(prow)
pdf('../holger_bastian/plot/volcano_wgii_cnv_metattest.pdf',width =8,height = 6)
print(prow)
dev.off()