...
for (i in 1:(length(colnames(rna_table))-1)) {
  ...
  if(length(which(is.na(usedmat[, gene])))<=10)
  {
    if(var(usedmat[, gene],na.rm = T)!=0)
    {
      ...
      wt=cor.test(usedmat$selected_term,usedmat$wgii,method = 'spearman')
      pvalue =wt$p.value
      coef = wt$estimate
...
write.table(Results, paste0(c("table/wgii_gene_exp_cor_",organ,".txt"),collapse = ''), quote=F, sep="\t", row.names = F)
}
