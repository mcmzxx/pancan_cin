...
for(fl in fls1){
  ...
  results[[tmpname]]=Results$Pathway[which(Results$Spearman_coef <= -0.31)]
}
consistent=table(unlist(results))

topPathway=names(consistent)[which(consistent>=7)]
consistent=consistent[topPathway]
consistent=sort(consistent,decreasing = T)
...
for(fl in fls2){
  ...
  results[[tmpname]]=Results$Pathway[which(Results$Spearman_coef <= -0.4)]
}
consistent=table(unlist(results))
...
pdf("plot/paradigm_order_by_scin_neg.pdf",width =6,height = 5)
print(mainplot_scin)
dev.off()


prow=plot_grid(mainplot_wgii,mainplot_scin, align = "v", axis = "l",ncol=1,rel_heights= c(1,1), labels = c("A","C"),label_x = -0.005)
pdf('plot/paradigm_order_by_cin_neg.pdf',width=9.3,height = 10.7)
